...
function convertDithering() {
  ...
  // 文字专用模式
  const isTextMode = document.getElementById('textOptimize')?.checked || false;
  const textThreshold = parseInt(document.getElementById('textThreshold').value);
  ...
  // 1. 文字模式专用处理
  if (isTextMode) {
    const processedData = processTextImage(currentImageData, document.getElementById('ditherMode').value,textThreshold);
    // const processedData = processTextImage(currentImageData);
    const finalImageData = decodeProcessedData(
      processedData, 
      canvas.width, 
      canvas.height, 
      document.getElementById('ditherMode').value
    );
...
function initEventHandlers() {
  ...
  document.getElementById("textThreshold").addEventListener("input", (e) => {
    document.getElementById("textThresholdValue").innerText = parseInt(e.target.value) //.toFixed(1);
    applyDither();
  });
}
