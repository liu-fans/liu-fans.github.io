...
// 新增文字优化处理函数
function processTextImage(imageData, mode,textThreshold) {
  const width = imageData.width;
  const height = imageData.height;
  const data = imageData.data;
  
  // // 1. 保护暗部细节的对比度调整
  // const safeContrast = 1.1; // 限制最大对比度
  // for (let i = 0; i < data.length; i += 4) {
  //   // 仅对中高亮度区域增强对比度
  //   if (data[i] > 100 || data[i+1] > 100 || data[i+2] > 100) {
  //     data[i] = Math.min(255, Math.max(0, (data[i] - 128) * safeContrast + 128));
  //     data[i+1] = Math.min(255, Math.max(0, (data[i+1] - 128) * safeContrast + 128));
  //     data[i+2] = Math.min(255, Math.max(0, (data[i+2] - 128) * safeContrast + 128));
  //   }
  // }
  ...
  if (mode === 'blackWhiteColor') {
    ...
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        ...
        const bit = luminance < threshold ? 0 : 1;
        
        const byteIndex = y * byteWidth + (x >> 3);
        const bitIndex = 7 - (x & 7);
        if (bit) processedData[byteIndex] |= (1 << bitIndex);
        else processedData[byteIndex] &= ~(1 << bitIndex);
      }
    }
    return processedData;
  ...
  // 3. 三色模式文字优化
    ...
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        ...
        const threshold = textThreshold;
        // 黑白层
        const blackWhiteBit = luminance < threshold ? 0 : 1;
        
        // 红色层：增强红色检测
        const isRed = (r > 150 && r > g*1.3 && r > b*1.3);
        const redWhiteBit = isRed ? 0 : 1;
        ...
        //   // 如果当前是白色但邻居是黑色，且原图是暗色 → 修复为黑色
        //   if (blackWhiteBit && prevLum < 120 && luminance < 140) {
        //     blackWhiteBit = 0;
        //   }
        // }
        ...
        if (blackWhiteBit) blackWhiteData[byteIdx] |= (1 << bitIdx);
        else blackWhiteData[byteIdx] &= ~(1 << bitIdx);
        
        if (redWhiteBit) redWhiteData[byteIdx] |= (1 << bitIdx);
        else redWhiteData[byteIdx] &= ~(1 << bitIdx);
      }
    }
  ...
  // 其他模式回退到原始处理
  return processImageData(imageData, mode);
}

// // 文字专用处理：跳过对比度增强
// function processTextImage(imageData) {
//   const data = imageData.data;
//   const threshold = 160; // 保留更多暗部细节
